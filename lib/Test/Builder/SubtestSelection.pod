...
This class is a subclass of L<Test::Builder> that overrides its C<subtest()>
method. It assumes that L<Test::Builder> subtests are used to group related
...
is the value of either the short C<-s> or long C<--subtest> option that have to
be supplied together as arguments to the test script. To do so separate the
arguments from prove's own arguments with the arisdottle (C<::>). Example:
